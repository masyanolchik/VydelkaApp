    ...
    LazyColumn(
        ...
        item {
            ...
            when (promoListState) {
                ...
                is Result.Success -> {
                    val rememberedList =
                        remember { (promoListState as Result.Success<List<Promo>>).data }
                    val rememberedAdapter = remember(openPromoScreenId) {
                        CarouselImageAdapter() {
                            openPromoScreenDialog = true
                            openPromoScreenId = it.promoId
                        }.apply {
                            submitList(
                                rememberedList.map {
                                    CarouselImageAdapter
                                        .CarouselData(it.name, it.promoImageSrc, it.id)
                                }
                            )
                        }
                    }
                                ...
                                SmallCategoryButton(
                                    ...
                                    navController.navigate(
                                        NavigationRoutes.getNavigationRoute(
                                            NavigationRoutes.CategoryRoute,
                                            it.category.id
                                        )
                                    )
                                }
            ...
            Divider()
            val isProductWatchHistoryEnabled by feedViewModel.isUserLoggedIn.collectAsStateWithLifecycle(
                initialValue = false
            )
                    ...
                    is Result.Success -> {
                        ...
                        productList.forEach {
                            ...
                            var isProductFavorited =
                                when (favoritesState) {
                                    is Result.Success -> (favoritesState as Result.Success<List<Product>>).data.map { it.id }
                                        .contains(it.id)
